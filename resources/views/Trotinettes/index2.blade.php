...
<!-- header section start -->
               ...
               <div class="row">
                  <div class="col-md-6">
                     <div class="box_main">
                        ...
                        <div class="image_2"><img src="/coverTrotinette/{{ $trotinette->image }}"></div>
                     </div>
                  </div>
                  <div class="col-md-6">
                     <h1 class="cycles_text">{{ $trotinette->nom }}</h1>
                     <h2>{{ $trotinette->categoryT->type }}</h2>
                     <p class="lorem_text">Couleur : {{ $trotinette->quantité }}</p>
                     <p class="lorem_text">Couleur : {{ $trotinette->couleur }}</p>
                     <p class="lorem_text">Poids : {{ $trotinette->poids }}</p>
                     <p class="lorem_text">Vitesse : {{ $trotinette->vitesse }}</p>

                     <div class="btn_main">
                        <div class="buy_bt"><a href="{{route('locations.create2',$trotinette->id)}}"> Louer </a></div>
                        <h4 class="price_text">Prix <span style=" color: #f7c17b"></span> <span style=" color: #325662">{{ $trotinette->prix_location }} DT</span></h4>
                     </div>
                  </div>
               </div>
            </div>
      ...
      <!-- Trotinettes section start -->
  <div class="news_section layout_padding">
         <div class="container">
            <h1 class="news_taital">Nos Trotinettes</h1>
            <p class="news_text">Choisissez parmis une large gammes de trotinettes selon vos préferences </p>
            <div class="news_section_2 layout_padding">
               <div class="row">
                 @foreach ($trotinettes as $trotinette)
                  <div class="col-sm-4">
                     <div class="box_main_1">
                        <div class="zoomout frame"><img src="{{ asset('Front')}}/images/img-8.png"></div>
                        <div class="padding_15">
                           <h2 class="speed_text">{{ $trotinette->nom }}</h2>
                           <h2 class="speed_text">Catégorie : {{ $trotinette->categoryT->type }}</h2>
                         
                           <div class="post_text">Prix  : {{ $trotinette->prix }} <span style="float: right;"></span></div>
                           <div class="post_text">Prix Location : {{ $trotinette->prix_location }} <span style="float: right;"></span></div>
                           <div class="post_text">En Stock : {{ $trotinette->quantite }} <span style="float: right;"></span></div>
                           <p class="long_text">Couleur : {{ $trotinette->couleur }}</p>
                           <p class="long_text">Poids : {{ $trotinette->poids }}</p>
                           <p class="long_text">Vitesse : {{ $trotinette->vitesse }}</p>
